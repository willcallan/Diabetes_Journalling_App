...
// Returns a FHIR observation based on the user's input
  ...
  client.create(obs)
    .then(
      function(res) {
        alert('Measurement posted.\nIt may take a few minutes for the graph at the bottom of the page to update.');
        clearInputs();
        refreshChart();
  ...
  client.request("Observation?" + query, {
    ...
    function(obs) {
      ...
      for (let i = 0; i < obs.length; i++) {
        ...
        // Mark what type of observation it is
        let code = obs[i].code.coding[0].code;
        if (code === '88365-2') {
          val.indexLabel = 'pre-meal';
        } else if (code === '87422-2') {
          val.indexLabel = 'post-meal';
        } else if (code === '6689-4') {
          val.indexLabel = '2h+ post-meal';
        }
        values.push(val);
      }
...
FHIR.oauth2.ready().then(function(cli) {
  client = cli;
  ...
  refreshChart();
  window.setInterval(refreshChart, 30000);
